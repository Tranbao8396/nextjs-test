...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('/api/users/change-password', { currentPassword, newPassword, confirmPassword });
      form.reset();
      setMessage({ type: 'success', text: response.data?.message || 'Password changed successfully.' });
    } catch (error) {
      setMessage({ type: 'danger', text: error.response?.data?.message || 'Unable to change password. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
