...
export default function DashboardPage() {
  ...
  const handlesubmit = async (e) => {
    ...
    if (!name || !submittedPass) {
      setResponseValid({ isSuccessful: false, message: 'Name and password are required.' });
      return;
    }
    ...
    try {
      ...
      if (req.status === 200) {
        setResponseValid({ isSuccessful: true, message: req.data?.message || 'Created' });
        body.reset();
      }
    } catch (e) {
      setResponseValid({
